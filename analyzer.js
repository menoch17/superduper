...
function analyzePacketData() {
    ...
    filteredPackets.forEach(packet => {
        ...
        if (srcIP && srcIP !== '' && !srcIP.startsWith('fd00:')) {
            ...
            ipAnalysis[srcIP].isSource = true;
            ipAnalysis[srcIP].packets++;
            ipAnalysis[srcIP].srcPackets++;
            ipAnalysis[srcIP].bytes += bytes;
            ipAnalysis[srcIP].srcBytes += bytes;
            ipAnalysis[srcIP].connections++;
        ...
        if (dstIP && dstIP !== '' && !dstIP.startsWith('fd00:')) {
            if (!ipAnalysis[dstIP]) {
                ipAnalysis[dstIP] = {
                    ...
                    dstBytes: 0,
                    ports: new Set(),
                    service: identifyService(dstIP, dstPort || effectivePort),
                    protocols: new Set(),
                    connections: 0
                };
            }
            ipAnalysis[dstIP].isDestination = true;
            ipAnalysis[dstIP].packets++;
            ipAnalysis[dstIP].dstPackets++;
            ipAnalysis[dstIP].bytes += bytes;
            ipAnalysis[dstIP].dstBytes += bytes;
            ipAnalysis[dstIP].connections++;
...
function displayPacketAnalysis(ipAnalysis, serviceStats, portStats, appDetection, durationStats, destinationStats, timelineStats) {
    ...
    let ipAnalysisHTML = `
        ...
        <div style="overflow-x: auto;">
            <table class="data-table" id="ipAnalysisTable" style="width: 100%; border-collapse: collapse;">
                <thead><tr style="background: var(--primary-color); color: white;">
                    <th style="padding: 12px; text-align: left;">WHOIS / IP</th>
                    <th style="padding: 12px; text-align: center;">Direction</th>
                    <th style="padding: 12px; text-align: left;">Service</th>
                    <th style="padding: 12px; text-align: right;">Packets</th>
                    <th style="padding: 12px; text-align: right;">Bytes</th>
                    <th style="padding: 12px; text-align: right;">Connections</th>
                    <th style="padding: 12px; text-align: left;">Ports</th>
                    <th style="padding: 12px; text-align: left;">Protocols</th>
                </tr></thead>
...
function filterIPAnalysis(filterType) {
    const rows = document.querySelectorAll('#ipAnalysisTable .ip-row');
    const buttons = document.querySelectorAll('[data-ip-filter]');

    // Update button states
    buttons.forEach(btn => {
        if (btn.getAttribute('data-ip-filter') === filterType) {
    ...
    rows.forEach(row => {
        ...
        if (filterType === 'all') {
            row.style.display = '';
        } else if (filterType === 'source' && (direction === 'source' || direction === 'both')) {
            row.style.display = '';
        } else if (filterType === 'destination' && (direction === 'destination' || direction === 'both')) {
            row.style.display = '';
        } else {
...
function displayCallAnalysis(analysis, mode = 'calls') {
    ...
    let contactsHTML = `
        <div style="margin-bottom: 8px; display: grid; grid-template-columns: 2fr 2fr 2fr 1fr 1fr; gap: 8px; font-size: 0.78rem; color: var(--text-secondary); text-transform: uppercase; letter-spacing: 0.04em;">
            <div><strong>Contact</strong></div>
            <div><strong>Name</strong></div>
            <div><strong>Carrier</strong></div>
            <div style="text-align: right;"><strong>Events</strong></div>
            <div style="text-align: right;"><strong>Total Duration</strong></div>
        </div>
    `;
    ...
    // Call Duration Trends Over Time (always shown, based on call records)
    const callSummaryForDuration = window.callAnalysisCallsSummary || analysis;
    const durationTrends = Array.from(callSummaryForDuration.durationByDay.entries())
        .sort((a, b) => new Date(a[0]) - new Date(b[0]));
    ...
    if (durationTrends.length === 0) {
        durationTrendsHTML += '<tr><td colspan="4" style="padding: 8px; color: var(--text-secondary);">No call duration data available.</td></tr>';
    }
    ...
    // Average Call Duration by Contact (always shown, based on call records)
    const contactsByAvgDuration = Array.from((window.callAnalysisCallsSummary || analysis).contacts.entries())
        .map(([number, data]) => ({
            number,
            ...data,
    ...
    if (contactsByAvgDuration.length === 0) {
        avgDurationHTML += '<tr><td colspan="4" style="padding: 10px; color: var(--text-secondary);">No call duration data available.</td></tr>';
    }
